 ...
 * }
 */
module.exports = class MywebpackUpload {
    constructor(options) {
    ...
    apply(compiler) {
        ...
        var onEmit = async (compilation, callback) => {
            var steps = [];
            async.forEach(Object.keys(compilation.assets), function (file, cb) {
                ...
                var outputPath = compilation.getPath(this.outputPath || compiler.outputPath),
                    ...
                    content = compilation.assets[file].source() || compilation.assets[file].content;
 ...
 */
async function fileTraverse(filePath, wpUploadOptions) {
            ...
            files.forEach(async function (filename) {
                ...
                fs.stat(filedir, async function (eror, stats) {
                    if (eror) {
                        ...
                        if (isFile) {
                            content = await readFileAsync(filedir);
                            var length = wpUploadOptions.extraPath.split('/').length;
                            var subDir = wpUploadOptions.extraPath.split('/')[length - 1];
                            var targetPath = filedir.substring(1);
                            var targetFile = subDir + filedir.split(wpUploadOptions.extraPath)[1];
                            filedir.split(wpUploadOptions.extraPath)[1] !== undefined
                                && _upload(
                                    wpUploadOptions.receiver,
                                    wpUploadOptions.to,
                                    wpUploadOptions.data,
                                    content,
                                    targetPath,
                                    targetFile,
                                    function (error, re) {
                        ...
                        if (isDir) {
                            fileTraverse(filedir, wpUploadOptions);//递归，如果是文件夹，就继续遍历该文件夹下面的文件
                        }
                    }
                })
            });
